...
class Sales extends Secure_area
{
	...
	function item_search()
	{
		...
		{
			$suggestions = $this->Item->get_item_search_suggestions($this->input->get('term'),0,'unit_price',$this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20, TRUE);
		...
		{
			$suggestions = $this->Item->get_item_search_suggestions_without_variations($this->input->get('term'),0,$this->config->item('number_of_items_per_page') ? (int)$this->config->item('number_of_items_per_page') : 20,'unit_price', TRUE);
			$suggestions = array_merge($suggestions, $this->Item_kit->get_item_kit_search_suggestions_sales_recv($this->input->get('term'),0,'unit_price', 100, TRUE));
	...
	function change_mode($mode = false, $redirect = false)
	{
		...
		if ($mode == 'store_account_payment')
		{
			...
			$this->cart->empty_items();
			$this->cart->add_item(new PHPPOSCartItemSale(array('scan' => $store_account_payment_item_id.'|FORCE_ITEM_ID|','cart' => $this->cart)));
			$this->cart->save();
		}
	...
	{		
		...
		if($markup_markdown && !$this->Location->get_info_for_key('disable_markup_markdown'))
		{
			...
			if (isset($markup_markdown_config[$this->input->post('payment_type')]) && $markup_markdown_config[$this->input->post('payment_type')])
			{
				...
				$fee_amount = to_currency_no_money($this->cart->get_total()*($fee_percent/100));
	...
	function edit_line_total($line)
	{
		...
		if(isset($min) && floatval($item->unit_price) < floatval($min))
		{
			$item->unit_price = $min;
			$data['warning'] = lang('sales_could_not_set_item_price_bellow_min')." ".to_currency($min);
		}

		if(isset($max) && floatval($item->unit_price) > floatval($max))
		{
			$item->unit_price = $max;
	...
	function edit_item($line)
	{
		...
		if($can_edit && isset($unit_price))
		{
			...
			if(isset($min) && floatval($unit_price) < floatval($min))
			{
				$item->unit_price = $min;
				$data['warning'] = lang('sales_could_not_set_item_price_bellow_min')." ".to_currency($min);
			}

			if(isset($max) && floatval($unit_price) > floatval($max))
			{
				$item->unit_price = $max;
				$data['warning'] = lang('sales_could_not_set_item_price_above_max')." ".to_currency($max);
			}
	...
	function complete()
	{
		...
		if($email_send === true){
			...
			$this->email->from($this->Location->get_info_for_key('email') ? $this->Location->get_info_for_key('email') : 'no-reply@mg.POS-STORE.com', $this->config->item('company'));
    ...
    $this->load->library("m_pdf");
		$pdf_content = $this->m_pdf->generate_pdf($receipt_data,TRUE);
  ...
  function cancel_sale()
  {
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				for($k = 2;$k<=$num_rows; $k++)
				{
					...
					{
						$item_to_add = new PHPPOSCartItemSale(array('unit_price' => $price, 'description' => $description,'discount' => $discount,'cart' => $this->cart,'scan' => $item_id,'quantity' => $quantity));
	...
	function get_attributes_values() {
		...
		function get_modifiers()
		{
			...
			echo '<input type="submit" class="btn btn-primary" /></form><script>$("#modifier_form").ajaxForm({target: "#register_container", beforeSubmit: function(){jQuery("#choose_modifiers").modal("hide");}, success: itemScannedSuccess});
</script>';
		}
}
?>