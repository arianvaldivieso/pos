...
class Inventory_summary extends Report
{
    ...
    // Variations
		    $query = 'SELECT X.*,Z.name
                    ...
                    SUM(COALESCE(phppos_location_item_variations.quantity, phppos_location_items.quantity, 0)) AS quantity,
    ...
    */
	...
	private function dataQuery()
	{
		...
		$this->db->select('SQL_CALC_FOUND_ROWS 1 as _h', false);
    ...
    $this->db->select('COALESCE('.$this->db->dbprefix('location_item_variations').'.unit_price, '
		...
		$this->db->select($sum_query.' as quantity');
            ...
            from (
              select
                ...
                (ifnull(q.quantity,0) + ifnull(v.variation_quantity,0)) * coalesce(v.v_cost_price, q.q_cost_price, phppos_items.cost_price, 0) as inventory_total,
                      -- coalesce(phppos_location_item_variations.cost_price, phppos_item_variations.cost_price, phppos_location_items.cost_price, phppos_items.cost_price, 0) as inventory_total,
                (ifnull(q.quantity,0) + ifnull(v.variation_quantity,0)) * coalesce(v.v_unit, q.q_unit, phppos_items.unit_price, 0) as inventory_sale_total,
                      -- coalesce(phppos_location_item_variations.unit_price, phppos_item_variations.unit_price, phppos_location_items.unit_price, phppos_items.unit_price, 0) as inventory_sale_total,
                q_unit,
                v_unit,
                q_cost_price,
                v_cost_price
              ...
              -- location_items
              left join (
                select phppos_location_items.item_id, phppos_location_items.location_id, sum(quantity) quantity -- ADDED milc
                  ,coalesce(phppos_location_items.cost_price, phppos_items.cost_price) q_cost_price  -- ADDED milc
                  ,coalesce(phppos_location_items.unit_price, phppos_items.unit_price) q_unit -- ADDED milc
                from phppos_location_items
                        join phppos_items on phppos_items.item_id = phppos_location_items.item_id -- ADDED milc
    ...
    // To echo '<pre>';print_r($this->db->last_query());exit; //milc
		return $result;
	}
}
?>